...
%%
%LOOP IN Runge-Kutta 4th Order Method
...
while num_turns < max_turns
    ...
    k2vel1 = h*q*(E(1) + ((vel(2,index-1) + k1vel2/2)*B(3) - (vel(3,index-1) + k1vel3/2)*B(2)))/m;
    k2vel2 = h*q*(E(2) + ((vel(3,index-1) + k1vel3/2)*B(1) - (vel(1,index-1) + k1vel1/2)*B(3)))/m;
    k2vel3 = h*q*(E(3) + ((vel(1,index-1) + k1vel1/2)*B(2) - (vel(2,index-1) + k1vel2/2)*B(1)))/m;
    
    k3vel1 = h*q*(E(1)/m + ((vel(2,index-1) + k2vel2/2)*B(3) - (vel(3,index-1) + k2vel3/2)*B(2)))/m;
    k3vel2 = h*q*(E(2)/m + ((vel(3,index-1) + k2vel3/2)*B(1) - (vel(1,index-1) + k2vel1/2)*B(3)))/m;
    k3vel3 = h*q*(E(3)/m + ((vel(1,index-1) + k2vel1/2)*B(2) - (vel(2,index-1) + k2vel2/2)*B(1)))/m;
    
    k4vel1 = h*q*(E(1)/m + ((vel(2,index-1) + k3vel2)*B(3) - (vel(3,index-1) + k3vel3)*B(2)))/m;
    k4vel2 = h*q*(E(2)/m + ((vel(3,index-1) + k3vel3)*B(1) - (vel(1,index-1) + k3vel1)*B(3)))/m;
    k4vel3 = h*q*(E(3)/m + ((vel(1,index-1) + k3vel1)*B(2) - (vel(2,index-1) + k3vel2)*B(1)))/m;
    
    vel(1, index) = vel(1, index-1) + k1vel1/6 + k2vel1/3 + k3vel1/3 + k4vel1/6;
    vel(2, index) = vel(2, index-1) + k1vel2/6 + k2vel2/3 + k3vel2/3 + k4vel2/6;
    vel(3, index) = vel(3, index-1) + k1vel3/6 + k2vel3/3 + k3vel3/3 + k4vel3/6;
...
title("Electron Trajectory using Runge-Kutta 4th Order");
...
title("Momentum Scaling in Runge-Kutta 4th Order");